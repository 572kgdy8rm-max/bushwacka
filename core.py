...
# ── POSITION SIZING (threshold-based, not linear) ─────────────────────────
...
def get_metadata(ticker: str) -> dict:
    try:
        ...
        return {"sector": sector, "earnings_date": earnings}
    except:
        return {"sector": "Unknown", "earnings_date": "Unknown"}
...
def analyze(ticker: str, spy_df=None) -> dict:
    ...
    # Per-metric scores — Max Drawdown REMOVED from Risk
    scores = {
    ...
    # Category scores — Risk now 4 metrics
    def cat(keys):
    ...
    risk_score     = cat(["Sharpe (1yr)","Sortino (1yr)","Volatility","Beta vs SPY"])
    ...
    return {
        ...
        "metrics": {
            ...
            "Risk":              {k: scores[k] for k in ["Sharpe (1yr)","Sortino (1yr)","Volatility","Beta vs SPY"]},
